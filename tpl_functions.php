...
/**
 * driver addition : used for displaying learning path at the top of the wiki
 * Author: Nuno Flores <nuno.flores@gmail.com>
 * @license Not Necessary
 */
require_once(DOKU_PLUGIN.'driver/syntax/core.php');
require_once(DOKU_INC.'inc/pluginutils.php');
function tpl_driver_showLearningPath() {
		if (plugin_isdisabled('driver')) return;
		driver_showLearningPath();
}

function tpl_driver_showLPathButton() {
	if (plugin_isdisabled('driver')) return;
	driver_showLPathMenu();
}

function tpl_driver_metaheaders() {
	if (plugin_isdisabled('driver')) return;
	driver_metaheaders();
}

function tpl_driver_sidePanels() {
	if (plugin_isdisabled('driver')) return;
	driver_sidePanels();
}
